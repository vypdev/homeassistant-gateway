...
export function topologyView(ctx: OverviewViewContext): TemplateResult {
  ...
  const node = (label: string, value: string) => html`<div class="card" style="text-align:center"><div class="topology-node"><strong>${label}</strong><div class="tag inline-chip ${value === 'ok' || value === 'ready' ? 'ok' : value === 'unknown' ? '' : 'warn'}">${ctx.statusText(value)}</div></div></div>`;
  return html`<section class="card" style="margin-top:14px"><div class="toolbar"><div><h2>${ctx.t('systemTopology')}</h2><p>${ctx.t('topologyDescription')}</p></div><span class="mono">${ctx.t('topologyIngress')} → ${ctx.t('topologyGateway')} → ${ctx.t('topologyUpstream')}</span></div><div class="topology-grid"><div class="card" style="text-align:center"><div class="topology-node"><strong>${ctx.t('topologyIngress')}</strong><div class="tag inline-chip ok">${ctx.t('trusted')}</div></div></div>${node(ctx.t('topologyGateway'), ctx.ready?.status ?? 'unknown')}${node(ctx.t('topologyCore'), status('core'))}${node(ctx.t('topologyRecorder'), status('recorder'))}${node(ctx.t('topologyLogbook'), status('logbook'))}</div></section>`;
}
