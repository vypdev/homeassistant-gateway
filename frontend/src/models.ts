...
export type OperatorPolicySaveResponse = { selected: string[] };
export type Client = {
  client_id: string;
  display_name: string;
  profile: Profile;
...
export type DevelopmentTraceStep = { phase: string; transport: string; status: string; duration_ms: number; command?: string | null; path?: string | null; attempt?: number; code?: string | null; detail?: string | null };
export type DevelopmentResult = { status: string; operation: string; duration_ms: number; count: number; data?: unknown; reason?: string | null; details?: Record<string, unknown> | null; trace?: DevelopmentTraceStep[] };
...
export type View = 'overview' | 'clients' | 'policy' | 'mcp' | 'audit' | 'development' | 'catalog';
