...
export const APP_STYLES = css`
    :host { --gateway-space-1: 4px; --gateway-space-2: 8px; --gateway-space-3: 12px; --gateway-space-4: 16px; --gateway-space-6: 24px; --gateway-space-8: 32px; --ha-canvas: #141414; --ha-primary: #009ac7; --ha-primary-hover: #18bcf2; --ha-primary-active: #006787; --ha-surface: #202020; --ha-surface-raised: #363636; --ha-surface-muted: #202020; --ha-border: #363636; --ha-text: #f3f3f3; --ha-text-secondary: #cccccc; --ha-text-muted: #989898; --ha-success: #00883c; --ha-warning: #f36d00; --ha-danger: #dc3146; --ha-radius-card: 12px; --ha-radius-control: 8px; --ha-radius-pill: 9999px; --ha-shadow-card: none; display: block; color: var(--ha-text); min-height: 100vh; font: 14px/1.6 Roboto, Noto, sans-serif; }
    * { box-sizing: border-box; }
    ...
    .shell.light .card-label { color: #607286; }
    ...
    .shell.light nav button:hover:not([aria-selected="true"]) { color: #3c4043; background: #f1f3f4; }
    ...
    .layout { position: relative; width: min(1280px, calc(100% - 32px)); margin: auto; padding: var(--gateway-space-6) 0 var(--gateway-space-8); }
    .tab-navigation { width: 100%; }
    main { min-width: 0; padding: 0 0 42px; }
    ...
    .content-status { display: flex; justify-content: flex-end; min-height: 32px; margin-bottom: 16px; }
    .topline { display: flex; justify-content: space-between; align-items: flex-start; gap: 20px; margin-bottom: 28px; }
    .eyebrow { color: var(--ha-primary); letter-spacing: .14em; text-transform: uppercase; font-size: 11px; font-weight: 800; }
    ...
    p { margin: 0; color: var(--ha-text-muted); }
    .status-pill { display: inline-flex; align-items: center; gap: 8px; border: 1px solid var(--ha-border); border-radius: 999px; padding: 7px 11px; color: var(--ha-success); background: color-mix(in srgb, var(--ha-success) 16%, var(--ha-surface)); white-space: nowrap; }
    ...
    .card-label { color: var(--ha-text-muted); font-size: 12px; }
    ...
    button.primary, button.secondary, button.danger { min-height: 40px; display: inline-flex; align-items: center; justify-content: center; gap: 8px; border: 1px solid transparent; border-radius: var(--ha-radius-pill); padding: 8px 16px; color: #ffffff; background: var(--ha-primary); cursor: pointer; font: 500 14px/1 Roboto, Noto, sans-serif; letter-spacing: .01em; box-shadow: none; transition: background-color 150ms ease, border-color 150ms ease, color 150ms ease; }
    .button-leading-icon { display: inline-flex; align-items: center; justify-content: center; width: 1.1em; margin-inline-end: 0; color: currentColor; font-size: 1.35em; font-weight: 400; line-height: 0; vertical-align: -1px; }
    .button-spinner { width: 14px; height: 14px; margin-inline-end: 0; border: 2px solid currentColor; border-inline-end-color: transparent; border-radius: 50%; animation: gateway-spin 700ms linear infinite; }
    ...
    .responsive-record { display: grid; gap: 11px; border: 1px solid var(--ha-border); border-radius: 12px; padding: 14px; background: #07152233; min-width: 0; }
    ...
    .responsive-field > span:first-child { color: var(--ha-text-muted); font-size: 12px; }
    ...
    td { color: var(--ha-text); }
    code, .mono { color: #9bdbff; font-family: ui-monospace, monospace; font-size: 12px; }
    .tag { display: inline-flex; align-items: center; min-height: 24px; color: var(--ha-text-secondary); background: var(--ha-surface-raised); border: 1px solid var(--ha-border); border-radius: var(--ha-radius-pill); padding: 3px 9px; margin: 2px 3px 2px 0; font-size: 12px; line-height: 1.2; }
    ...
    .ok { color: var(--ha-success); } .warn { color: var(--ha-warning); } .bad { color: var(--ha-danger); }
    ...
    .token { word-break: break-all; border: 1px dashed var(--ha-primary); border-radius: 10px; padding: 14px; color: #b8ecff; background: #052a4055; margin: 12px 0; }
    ...
    .empty { padding: 28px 10px; text-align: center; color: var(--ha-text-muted); }
    ...
    .pack-grid small { color: var(--ha-text-secondary); line-height: 1.4; }
    ...
    .capability-option:hover { border-color: var(--ha-primary); }
    .capability-option input { width: auto; margin-top: 3px; }
    .capability-option strong { display: block; color: var(--ha-text); font-size: 12px; }
    .capability-option small { display: block; color: var(--ha-text-muted); margin-top: 2px; }
    ...
    .policy-summary-item { display: grid; gap: 4px; min-height: 72px; padding: 12px 13px; border: 1px solid var(--ha-border); border-radius: 10px; background: #07152266; }
    .policy-summary-item strong { color: #d7f5ff; font-size: 22px; line-height: 1; }
    .policy-summary-item span { color: var(--ha-text-muted); font-size: 12px; line-height: 1.35; }
    .operator-policy-change-note { margin: 14px 0 18px; }
    .operator-service-groups { display: grid; gap: 30px; }
    .operator-service-group { display: grid; gap: 13px; padding: 16px; border: 1px solid var(--ha-border); border-radius: 14px; background: #07152233; }
    ...
    .operator-service-option:hover { border-color: var(--ha-primary); background: #0b243699; }
    .operator-service-option input { width: auto; margin-top: 3px; }
    .operator-service-option strong { display: block; color: var(--ha-text); font-size: 13px; line-height: 1.35; }
    .operator-service-option small { display: block; margin-top: 4px; color: var(--ha-text-muted); line-height: 1.4; }
    ...
    .result-row { display: flex; justify-content: space-between; gap: 14px; align-items: flex-start; border: 1px solid var(--ha-border); border-radius: 10px; padding: 12px 13px; background: #07152233; }
    ...
    @media (max-width: 900px) { .navigation-tabs { height: 56px; } .navigation-tabs .ui-tab { min-height: 56px; } }
