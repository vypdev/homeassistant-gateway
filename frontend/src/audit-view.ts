...
export function auditView(ctx: AuditViewContext): TemplateResult {
  return html`<div class="card"><div class="toolbar"><div><h2>${ctx.t('auditEvents')}</h2><p>${ctx.t('auditNotStored')}</p></div><div><label for="audit-decision-filter">${ctx.t('decision')}</label><select id="audit-decision-filter" aria-label=${ctx.t('decision')} @change=${(event: Event) => ctx.loadAudit((event.target as HTMLSelectElement).value)}><option value="">${ctx.t('allDecisions')}</option><option value="allowed">${ctx.t('allowed')}</option><option value="denied">${ctx.t('denied')}</option><option value="approval_required">${ctx.t('approvalRequiredOption')}</option></select></div></div>${ctx.audit.length ? html`
    ...
    </article>`)}</div>` : html`<div class="empty">${ctx.t('noAudit')}</div>`}</div>`;
}
