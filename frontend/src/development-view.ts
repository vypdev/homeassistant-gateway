...
import { gatewayButton, gatewayCard, gatewayEmptyState, gatewayResultRow, gatewayStatus, gatewayTextField } from './ui';
...
export function developmentView(ctx: DevelopmentViewContext): TemplateResult {
  ...
  return html`<div class="dev-grid">
    ${gatewayCard(html`
      ...
      <div class="result-list">${catalog?.operations.map((operation) => gatewayResultRow(html`<strong>${ctx.operationText(operation.name, 'Label', operation.label)}</strong><br><span class="muted">${ctx.operationText(operation.name, 'Description', operation.description)}</span>`, gatewayButton({ label: ctx.t('run'), variant: 'secondary', disabled: ctx.busy || !catalog.enabled, onClick: () => ctx.runOperation(operation.name) })) )}</div>
      `, 'observer-probes-card')}
    ${gatewayCard(html`
      <div class="toolbar"><div><h2>${ctx.t('executionEvidence')}</h2><p>${ctx.t('countLatency')}</p></div><div class="evidence-toolbar"><div class="evidence-metrics">${ctx.progress.total ? gatewayStatus(`${ctx.progress.completed}/${ctx.progress.total}`, ctx.progress.status === 'error' ? 'bad' : ctx.progress.status === 'warning' ? 'warn' : '') : ''}${ctx.results.length ? gatewayStatus(`${ctx.results.length} ${ctx.t('result')}`) : ''}${ctx.results.filter((result) => isProblemStatus(result.status)).length ? gatewayStatus(`${ctx.results.filter((result) => isProblemStatus(result.status)).length} ${ctx.t('problemReports')}`, 'warn') : ''}</div><div class="evidence-actions">${gatewayButton({ label: ctx.t('copyProblemReports'), variant: 'secondary', disabled: !ctx.results.some((result) => isProblemStatus(result.status)), onClick: () => ctx.copyProblemReports() })}${gatewayButton({ label: ctx.t('exportDiagnostic'), variant: 'secondary', onClick: () => ctx.download() })}</div></div></div>
