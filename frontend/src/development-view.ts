...
import type { DevelopmentCatalog, DevelopmentPack, DevelopmentReport, DevelopmentResult } from './models';
...
export function developmentView(ctx: DevelopmentViewContext): TemplateResult {
  ...
  return html`<div class="dev-grid">
    ...
    <div class="card">
      <div class="toolbar"><div><h2>${ctx.t('executionEvidence')}</h2><p>${ctx.t('countLatency')}</p></div><div>${ctx.progress.total ? html`<span class="tag ${ctx.progress.status === 'error' ? 'bad' : ctx.progress.status === 'warning' ? 'warn' : ''}" aria-live="polite">${ctx.progress.completed}/${ctx.progress.total}</span>` : ''}${ctx.results.length ? html`<span class="tag">${ctx.results.length} ${ctx.t('result')}</span>` : ''}<button class="secondary" @click=${() => ctx.download()}>${ctx.t('exportDiagnostic')}</button></div></div>
      ${ctx.results.length ? html`<div class="result-list">${ctx.results.map((result: DevelopmentResult) => html`<div class="result-row"><span><strong>${ctx.operationText(result.operation, 'Label', result.operation)}</strong> <span class=${result.status === 'ok' ? 'ok' : result.status === 'warning' ? 'warn' : 'bad'}>${ctx.statusText(result.status)}</span>${result.reason ? html`<br><span class="muted">${result.reason}</span>` : ''}</span><span class="mono">${result.count} ${ctx.t('devItems')} · ${result.duration_ms} ${ctx.t('devMilliseconds')} ${result.status !== 'ok' ? html`<button class="secondary" @click=${() => ctx.copyDiagnostic(result)}>${ctx.t('copyDiagnostic')}</button><button class="secondary" @click=${() => ctx.retry(result.operation)}>${ctx.t('retry')}</button>` : ''}</span></div>`)}</div><pre class="dev-output">${JSON.stringify(ctx.output, null, 2)}</pre>` : html`<div class="empty">${ctx.t('exactResponse')}</div>`}
