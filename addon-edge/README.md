...
built by GitHub Actions from the `main` branch and published as one
...
- `ghcr.io/vypdev/homeassistant-gateway-edge:0.5.32-edge-2` (versioned image)
...
Edge embeds the Storybook catalog generated from the same `main` source tree. Open it through the Supervisor Ingress URL at `/catalog/` (for example, `http://<ingress>/catalog/`). Stable does not include this catalog.
