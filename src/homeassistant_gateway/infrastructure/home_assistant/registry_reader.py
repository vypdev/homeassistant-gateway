...
class SupervisorRegistryReader:
    ...
    def _template_registry(self: Any, resource: str) -> list[dict[str, Any]]:
        templates = {
            ...
            "entity_registry": "{% set ns = namespace(items=[]) %}{% for item in states %}{% set ns.items = ns.items + [{'entity_id': item.entity_id, 'state': item.state, 'attributes': item.attributes, 'device_id': device_id(item.entity_id)}] %}{% endfor %}{{ ns.items | tojson }}",
        }
